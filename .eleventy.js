...
export default async function (eleventyConfig) {
	...
	const cssTemplate = fs.readFileSync(
		path.resolve("theme/css/", "bundle.njk"),
		"utf-8"
	);
	const jsTemplate = fs.readFileSync(
		path.resolve("theme/js/", "bundle.njk"),
		"utf-8"
	);
	const robotsTemplate = fs.readFileSync(
		path.resolve("elva/templates/", "robots.njk"),
		"utf-8"
	);
	const sitemapTemplate = fs.readFileSync(
		path.resolve("elva/templates/", "sitemap.njk"),
		"utf-8"
	);
	...
	const feedTemplate = fs.readFileSync(
		path.resolve("elva/templates/", "feed.njk"),
		"utf-8"
	);
	const feedXSLTemplate = fs.readFileSync(
		path.resolve("elva/templates/", "feed.xsl.njk"),
		"utf-8"
	);
	const feedJSONTemplate = fs.readFileSync(
		path.resolve("elva/templates/", "feed.json.njk"),
		"utf-8"
	);
	const manifestTemplate = fs.readFileSync(
		path.resolve("elva/templates/", "manifest.njk"),
		"utf-8"
	);

	for (let [key, locale] of Object.entries(locales)) {
		eleventyConfig.addTemplate(key + "-feed.njk", feedTemplate, {
			lang: key,
		});
		eleventyConfig.addTemplate(key + "-feed.xsl.njk", feedXSLTemplate, {
			lang: key,
		});
		eleventyConfig.addTemplate(key + "-feed.json.njk", feedJSONTemplate, {
			lang: key,
		});
		eleventyConfig.addTemplate(key + "-manifest.njk", manifestTemplate, {
			lang: key,
		});
	}
	...
	eleventyConfig.addPassthroughCopy({
		"./content/assets/files": "./assets/files",
	});
	eleventyConfig.addPassthroughCopy({
		"./content/assets/img": "./assets/img",
	});
